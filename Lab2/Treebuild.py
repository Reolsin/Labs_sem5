    ...
    if node == None:
        ...
        if node.value == '.':
            ...
            while len(s) != 0:
                next_pos.update(s)
                s = set()
                [s.update(Tree_passage(node.left, string, pos)) for pos in s]
            next_pos.add(i)
...
str = 'das'
pos = Tree_passage(tree, str, 0)
print(pos)

NFA = NFA_builder(tree)
DFA = DFA_builder(NFA, 'asd')
min2 = min_DFA2(DFA, 'asd')
DFA_passage(min2, str)
